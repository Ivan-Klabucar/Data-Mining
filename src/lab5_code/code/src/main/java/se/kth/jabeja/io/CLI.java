...
public class CLI {
  ...
  @Option(name = "-diffAnnealing", usage = "Don't use linear annealing, but rather the exponential annealing mechanism described in task 2. If this option is set to true remember to set delta somewhere in range [.7, 0.99].")
  private boolean DIFFANNEALING = false;

  @Option(name = "-rstAnnealing", usage = "Reset annealing everytime the algorithm converges.")
  private boolean RSTANNEALING = false;

  @Option(name = "-customAccFunc", usage = "Use custom acceptance function for bonus task, and set it's value C (double). This flag automatically turns this flag on -diffAnnealing. Good value to use [4,10].")
  private Double CUSTOMACCFUNC = null;
  ...
  public Config parseArgs(String[] args) throws FileNotFoundException {
    ...
    return new Config().setRandNeighborsSampleSize(randNeighborsSampleSize)
            ...
            .setAlpha(ALPHA)
            .setDiffAnnealing(DIFFANNEALING)
            .setRstAnnealing(RSTANNEALING)
            .setCustomAccFunc(CUSTOMACCFUNC);
  }
}
